...
locals {
  ...
  # Check if ACR secret exists (which indicates credentials are configured)
  acr_secret_exists = length(aws_secretsmanager_secret.acr_credentials) > 0
  # Filter registries: exclude ACR-related ones if secret doesn't exist
  ecr_pull_through_cache_registries = local.acr_secret_exists ? var.ecr_pull_through_cache_upstream_registries : [for reg in var.ecr_pull_through_cache_upstream_registries : reg if reg != var.acr_registry_url && reg != "uniquecr"]
}
...
resource "aws_ecr_pull_through_cache_rule" "main" {
  ...
  # Support both the full ACR URL and the short alias "uniquecr"
  credential_arn = (each.value == var.acr_registry_url || each.value == "uniquecr") && local.acr_secret_exists ? aws_secretsmanager_secret.acr_credentials[0].arn : null
