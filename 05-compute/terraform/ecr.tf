...
resource "aws_ecr_pull_through_cache_rule" "main" {
  for_each = toset(var.ecr_pull_through_cache_upstream_registries)
  ...
  credential_arn = (each.value == var.acr_registry_url || each.value == "uniquecr") && length(aws_secretsmanager_secret.acr_credentials) > 0 ? aws_secretsmanager_secret.acr_credentials[0].arn : null
