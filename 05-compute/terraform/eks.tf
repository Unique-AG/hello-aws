...
# Using hardcoded AWS EKS OIDC thumbprint as per AWS best practices
...
resource "aws_iam_openid_connect_provider" "eks" {
  ...
  tags = merge(local.tags, {
    Name = "${module.naming.id}-eks-oidc"
  })
}
...
resource "aws_eks_cluster" "main" {
  ...
  # The cluster is already configured correctly, so ignore changes to avoid false update attempts
  lifecycle {
    ignore_changes = [
