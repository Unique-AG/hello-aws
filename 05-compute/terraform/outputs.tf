...
# Cross-Account IAM Role Output (for connectivity account)
output "connectivity_account_read_only_role_arn" {
  description = "ARN of the IAM role that allows the connectivity account to create Transit Gateway attachments"
  value       = aws_iam_role.connectivity_account_read_only.arn
}

# Transit Gateway Attachment Outputs
output "transit_gateway_attachment_id" {
  description = "ID of the Transit Gateway VPC attachment"
  value       = try(aws_ec2_transit_gateway_vpc_attachment.eks[0].id, null)
}

output "transit_gateway_attachment_arn" {
  description = "ARN of the Transit Gateway VPC attachment"
  value       = try(aws_ec2_transit_gateway_vpc_attachment.eks[0].arn, null)
}
