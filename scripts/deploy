...
"${SCRIPT_DIR}/restore-scheduled-secrets.sh"
echo -e "${BLUE}   Checking for pending KMS key deletions...${NC}"
"${SCRIPT_DIR}/cancel-pending-kms-deletions.sh"
echo ""
...
#######################################
# Step 4.5: Fix IAM Instance Profile Association (Infrastructure layer only)
#######################################
# This step fixes the IAM instance profile association for the management server
# when Terraform recreates the IAM role/profile but the running instance still
# has the old (deleted) association, causing SSM connection failures.
if [[ "$LAYER" == "infrastructure" ]]; then
  echo -e "${YELLOW}🔧 Step 4.5: Fixing IAM instance profile association (if needed)...${NC}"
  if "${SCRIPT_DIR}/fix-iam-instance-profile.sh" "${TERRAFORM_DIR}" "${ENV}"; then
    echo -e "${GREEN}✅ IAM profile check completed${NC}"
  else
    echo -e "${YELLOW}⚠️  IAM profile fix had issues (non-critical)${NC}"
  fi
  echo ""
fi
