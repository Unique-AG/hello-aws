...
import { deleteAssistant } from '../../../lib/chat';
...
test('@smoke browser — create a Space V2', async ({ page }) => {
  ...
  try {
    ...
    assistantId = body?.data?.createAssistant?.id;
    expect(assistantId, 'created assistant id').toBeTruthy();
  } finally {
    // Don't leak a Space into the shared tenant on every run.
    if (assistantId) await deleteAssistant(getBrowserUserToken(), assistantId);
  }
});
