#######################################
# Bastion and Management Server
#######################################
#
# Provides secure access to private resources:
# - AWS Systems Manager Session Manager (bastion alternative)
# - EC2 Management Server (optional jump server)
#
# Session Manager provides secure, audited access without
# requiring a public IP or SSH keys.
#######################################
...
# without requiring SSH keys, bastion hosts, or public IPs.
# This is the recommended AWS approach for secure access.
#
# VPC endpoints for SSM are created in vpc-endpoints.tf
# This file creates the IAM roles and EC2 instance for Session Manager access.
#######################################
...
#######################################
#
# Optional EC2 instance for management tasks:
# - Can be accessed via Session Manager (recommended)
# - Or via SSH from public subnet (if enabled)
# - Used for administrative tasks, troubleshooting, etc.
#######################################
...
resource "aws_security_group" "management_server" {
  ...
  # Allow outbound HTTPS to internet (for package updates, GitHub, tool downloads)
  # Management server needs internet access for yum updates, GitHub CLI, kubectl, etc.
  # Traffic routes through NAT Gateway (management server is in private subnet)
  egress {
    description = "Allow HTTPS to internet (for package updates, GitHub, tools)"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
  ...
  # Allow outbound SSH to internet (for GitHub CLI git operations via SSH)
  # GitHub CLI uses SSH protocol by default for git operations
  # Traffic routes through NAT Gateway (management server is in private subnet)
  egress {
    description = "Allow SSH to internet (for GitHub CLI git operations)"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
