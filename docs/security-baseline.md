...
| CKV_AWS_109, CKV_AWS_111, CKV_AWS_356 | `aws_iam_policy_document.podvm_import` | VM Import/Export needs `ec2:RegisterImage`, `CopySnapshot` and `ModifySnapshotAttribute` on `*`: each creates the resource it names, so there is no ARN to scope to at call time. S3 and KMS access in the same policy **is** scoped, to the staging bucket and the general key. **Compensating controls:** the role trusts only `vmie.amazonaws.com` with the required `sts:ExternalId` **and** an `aws:SourceAccount` condition, and it exists only while `enable_podvm_image_import` is set. |
| CKV_AWS_18, CKV_AWS_144, CKV2_AWS_62 | `aws_s3_bucket.podvm_import` | Access logging, cross-region replication and event notifications on a staging bucket that holds one disk image for the minutes an import takes, then expires it after 7 days. Same rationale as the other buckets in this table. **Compensating controls:** KMS-encrypted, versioned, public access blocked, and created only while `enable_podvm_image_import` is set. |
| CKV_AWS_21, CKV_AWS_145, CKV2_AWS_6, CKV2_AWS_61 | `aws_s3_bucket.podvm_import` | Versioning, KMS encryption, the public access block and a lifecycle rule are all configured, each as its own `count`-gated resource in `podvm-import.tf`. Checkov's graph check cannot link a `count`-indexed bucket to its `count`-indexed companions, so it reads all four as absent — the same false positive it produces for conditionally created security groups. The layer's unconditional buckets do not trip it. Verified by inspection. |
