...
| CKV_AWS_107, CKV_AWS_108, CKV_AWS_109, CKV_AWS_110, CKV_AWS_111, CKV_AWS_356 | `aws_iam_policy_document.github_actions_deploy` | This is the role CI assumes to apply every layer, so it necessarily carries IAM, KMS and S3 write breadth: a role that can create the roles and keys the landing zone needs cannot be scoped away from permissions management or privilege escalation without ceasing to do its job. Surfaced only once the gate began scanning `use_oidc`-gated resources. **Compensating controls:** assumable solely via GitHub OIDC restricted to this repository, no long-lived credentials, and every apply runs from a reviewed, merged commit. **Remediation:** split into per-layer roles so a compromised workflow cannot reach the whole account. |
...
| CKV_AWS_290, CKV_AWS_355 | `aws_iam_role_policy.github_runners` | The ENI actions (`CreateNetworkInterface`, `DescribeNetworkInterfaces`, `DeleteNetworkInterface`, `Describe*`) are AWS's documented requirement for running CodeBuild inside a VPC and cannot be resource-scoped — the interfaces do not exist until the build starts. Log writes in the same policy **are** scoped, to this account's log groups. **Remediation:** add an `ec2:Subnet` / `ec2:AuthorizedService` condition on `CreateNetworkInterfacePermission` per AWS's VPC guidance. |
| CKV_AWS_61 | `aws_iam_role.connectivity_account` | Cross-account trust for the connectivity account's Terraform execution role, narrowed by `aws:PrincipalOrgID` and an `ArnLike` on `aws:PrincipalArn`. **Note:** that `ArnLike` list begins with `role/*-terraform-execution` and then also allows `role/*` and `user/*`, which makes the narrowing ineffective — the effective trust is any principal in that account within the organization. **Remediation:** drop the `role/*` and `user/*` entries so only the terraform execution role can assume it. |
...
| CKV2_AWS_5 | `aws_security_group.github_runners` | Attached to the CodeBuild project through `vpc_config.security_group_ids` (github-runners.tf). Checkov's graph check does not follow a CodeBuild VPC attachment, only EC2 instances and ENIs, so it reads the group as orphaned. Verified by inspection. |
...
| CKV2_AWS_5 | `aws_security_group.peer_pods` | The group is attached by `cloud-api-adaptor` at pod-VM launch (`AWS_SG_IDS` in the peer-pods ConfigMap), not by a Terraform resource, so checkov's graph check sees no attachment. Pod VMs are created and destroyed per sandbox; Terraform owns the group, the adaptor owns the instances. |
